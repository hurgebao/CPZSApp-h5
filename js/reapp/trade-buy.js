...
function countEnableNum() {
	var orderPrice = mui('#orderPrice')[0].value;
//	console.log('trade-buy countEnableNum....'+orderPrice);
	if(orderPrice == '' || parseFloat(orderPrice) == 0) {
		//设置 快选仓位
		setQSelectNum(0);
		return false;
...
//	console.log('trade-buy currentEnableAmt....'+currentEnableAmt);
	if(currentEnableAmt == '') {
		//重新查询行情
		queryMarketInfo();
		return false;
...
//	console.log(stockCode+'  '+stockName);
	...
	//判断子页面是否已存在, 如果存在 销毁
	var subPage = plus.webview.getWebviewById('trade-buy-marketlist.html');
	if(subPage != null) {
...
function toOpenMarketWindow(){
	//子页面ID
	var id = 'trade-buy-marketlist.html';
	//判断子页面是否已存在, 如果存在 销毁
...
/** 模糊查询股票列表 */
function queryStockListsByVal(stockCodeVal){
	/** 发送读取行情 */
...
//	    	console.log('xhr_svn status:'+xhr_svn.status+'  '+stockListUrl);
	        if (xhr_svn.status == 200) {
	        	var stockContent = xhr_svn.responseText;
//	        	console.log(marketUrl+'模糊查询股票列表:'+stockContent);
	        	if(stockContent == null || stockContent == undefined) {
					return ;
	        	}
	        	var listMaps = JSON.parse(stockContent);
	        	//循环数组
				var contentStr = '';
				if(listMaps.length > 0) {
					for(var i = 0;i < listMaps.length; i++) {
						var stockMap = listMaps[i];
						var strList = '<li class="mui-table-view-cell"><a name="popoverA" stockCode="'+stockMap.stock_code+'" stockName="'+stockMap.stock_name+'" >'+stockMap.stock_code+' &nbsp; '+stockMap.stock_name+'</a></li>';
					    contentStr = contentStr + strList;
					}
				}
				mui('#stockLists')[0].innerHTML = contentStr;
				
				//显示
				mui("#topPopoverStockList").popover('show');
				/*if(listMaps.length == 1) {
					mui('#stockCode')[0].value = listMaps[0].stock_code;
					mui('#stockNameHid')[0].value = listMaps[0].stock_name;
					mui('#stockName')[0].innerHTML = listMaps[0].stock_name;
					//重新读取行情
					queryMarketInfo();
					//隐藏 弹出菜单
	...
	// 下发请求行情 
	xhr_svn.open("GET", stockListUrl+stockCodeVal);//这里的地址是上面json文件的地址   
	xhr_svn.send(); 
	

	//取项目后台
	/*var params = queryStockListsByValParams(stockCodeVal);
	mui.web_query_post('stockMarket/queryStockListsByVal', params, function(data){
//		console.log('queryStockListsByVal res: '+JSON.stringify(data));
		if(data.returnCode == 0){
...
function openKeyWindow(){
	var tradeBuyKey = plus.webview.getWebviewById('trade-buy-key.html');
	if(tradeBuyKey != null) {
		tradeBuyKey.evalJS("openKeyWindow()");
	}
}
/** 关闭键盘窗口 */
function closeKeyWindow(){
	var tradeBuyKey = plus.webview.getWebviewById('trade-buy-key.html');
	if(tradeBuyKey != null) {
		tradeBuyKey.evalJS("closeKeyWindow()");
	}
}

/* 设置股票开头 */
function setKeyItemQ(stockCodeVal) {
	mui('#stockCode')[0].value = stockCodeVal;
	/** 点击 自定义 加载行情  */ 
	loadStockMarket();
}
/* 点击单个字母或数字键 */
function setKeyItemSet(stockCodeVal) {
	var stockCode = mui('#stockCode')[0].value;
//	if(stockCode.length < 6) {
		mui('#stockCode')[0].value = stockCode + stockCodeVal;
//	}
	var stockCodeCurr = mui('#stockCode')[0].value;
	if(stockCodeCurr.length == 6) {
		/** 设置Gif股票 */
		setSubStockCode();
	}
	/** 点击 自定义 加载行情  */ 
...
/* 清空按键 */
function setKeyItemClear() {
	mui('#stockCode')[0].value = '';
	/** 点击 自定义 加载行情  */ 
	loadStockMarket();
}
/* 点击退格按钮 */
function setKeyItemBack() {
	var stockCode = mui('#stockCode')[0].value;
	if(stockCode.length > 1) {
		mui('#stockCode')[0].value = stockCode.substr(0, stockCode.length-1);
	} else {
		mui('#stockCode')[0].value = '';
	}
	/** 点击 自定义 加载行情  */ 
...
/** 设置Gif股票 */
function setSubStockCode(){
	var tradeBuyKey = plus.webview.getWebviewById('trade-buy-key.html');
	var stockCode = mui('#stockCode')[0].value;
	var stockName = mui('#stockName')[0].value;
//	console.log('tradeBuyKey:'+tradeBuyKey);
	if(tradeBuyKey != null) {
		tradeBuyKey.evalJS("setSubStockCode('"+stockCode+"','"+stockName+"')");
	}
}
